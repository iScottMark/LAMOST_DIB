...
from cis import CIS
# from measure.measure import measure_dib5780
from measure.measure import measure_dib5780_lite
from measure.measure import measure_dib6614
...
# GLOBAL VARIABLES
work_dir = os.getenv('HOME') + '/' + 'Project3/lamost_dib/'
data_config_path = work_dir + 'config/data.yml'
dib_config_path = work_dir + 'config/dib.yml'
neighbor_config_path = work_dir + 'config/neighbor.yml'
...
def get_file_paths(arg_start: int, arg_end: int, config_path: str = data_config_path) -> tuple:
    ...
    log_file = os.path.join(home, work_dir, output_dir, f'{output_prefix}_{arg_start}_{arg_end}.log')
    csv_file = os.path.join(home, work_dir, output_dir, f'{output_prefix}_{arg_start}_{arg_end}.csv')
    h5_file = os.path.join(home, work_dir, output_dir, f'{output_prefix}_{arg_start}_{arg_end}.h5')
...
def main(dib_name: str = '5780', is_debug: bool = False, is_save_h5: bool = True):
    ...
    # -------------------------------------------------------------------------
    # Build target catalog and reference catalog
...
if __name__ == '__main__':
    ...
    # main()
    main(dib_name='5780', is_debug=False, is_save_h5=True)
